...
export function TenantHeader({ config }: TenantHeaderProps) {
  const { businessName, subtitle, logoUrl } = config

  return (
    <header
      className="sticky top-0 z-40 w-full bg-[var(--color-background)]/80 backdrop-blur-md border-b border-black/10"
    ...
    >
      <div className="max-w-[var(--container-width)] mx-auto px-4 h-[var(--header-height)] flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          <button className="font-mono text-sm hover:underline decoration-2 underline-offset-4">
            COLLECTIONS
          </button>
          <button className="font-mono text-sm hover:underline decoration-2 underline-offset-4">
            ABOUT
