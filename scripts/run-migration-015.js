...
async function runMigration() {
  ...
  const SUPABASE_URL =
    process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://lmqysqapqbttmyheuejo.supabase.co'
  ...
  const migrationPath = path.join(
    __dirname,
    '../db/supabase/migrations/015_add_tenant_template_theme.sql',
  )
  let sql = fs.readFileSync(migrationPath, 'utf8')

  // Remove minimal-demo INSERT (use only 'sky' tenant)
  sql = sql.replace(
    /-- Create a third demo tenant[\s\S]*ON CONFLICT[\s\S]*updated_at = NOW\(\);/m,
    '',
  )
