  ...
  for (const file of imageFiles) {
    ...
    try {
      ...
      while (processedBuffer.length > MAX_SIZE_KB * 1024 && quality > 10) {
        console.log(`  Size ${Math.round(processedBuffer.length / 1024)}KB > ${MAX_SIZE_KB}KB. Reducing quality to ${quality - 10}...`);
        quality -= 10;
      ...
      if (processedBuffer.length > MAX_SIZE_KB * 1024) {
        console.warn(`  Warning: Could not get under ${MAX_SIZE_KB}KB even with quality ${quality}. Size: ${Math.round(processedBuffer.length / 1024)}KB`);
      }
