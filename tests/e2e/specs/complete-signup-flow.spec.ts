...
test.describe('Complete Signup & Onboarding Flow - Happy Path', () => {
  ...
  test('should allow user to navigate back and forth in wizard', async ({ page, dbCleanup }) => {
    // ARRANGE: Generate test data
    const testData = generateTestData('navigation-test')
    const testLogoPath = path.join(__dirname, '../fixtures/files/test-logo.png')
    ...
    // Step 1: Upload logo
    await wizard.uploadLogo(testLogoPath)
    await wizard.clickContinue()

    // Step 2: Colors
    await expect(page.locator('text=/Elige tus colores/')).toBeVisible()

    // Go back to Step 1
    await wizard.clickBack()
    await expect(page.locator('text=/Sube tu logo/')).toBeVisible()
    ...
    // Continue to Step 3
    await wizard.clickContinue()
    await expect(page.locator('text=/Agrega productos/')).toBeVisible()
    ...
    await wizard.clickContinue() // Step 4: Preview
    await wizard.clickContinue() // Step 5: Activation
  ...
  test('should add multiple products during onboarding', async ({ page, dbCleanup }) => {
    // ARRANGE: Generate test data with many products
    const testData = generateTestData('many-products')

    const manyProducts: ProductData[] = [
      { name: 'Producto 1', price: 10.00, category: 'Cat A', stock: 100 },
      { name: 'Producto 2', price: 20.00, category: 'Cat B', stock: 50 },
      { name: 'Producto 3', price: 30.00, category: 'Cat C', stock: 25 },
      { name: 'Producto 4', price: 40.00, category: 'Cat D', stock: 10 },
      { name: 'Producto 5', price: 50.00, category: 'Cat E', stock: 5 },
    ]

    // ACT: Complete signup and onboarding
    const signupPage = new SignupPage(page)
    await signupPage.navigate()
    await signupPage.fillForm(testData)
    await signupPage.submitAndWaitForOnboarding()
    ...
    // Complete onboarding with multiple products
    const storefrontPage = await wizard.completeOnboarding({
