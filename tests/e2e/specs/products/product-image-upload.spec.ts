...
test.describe('Product Image Upload (SKY-44)', () => {
  ...
  test('should reject non-image file types', async ({ page }) => {
    const productName = `Invalid File Test ${Date.now()}`

    // Step 1: Create a temporary text file for testing
    await test.step('Attempt upload of invalid file type', async () => {
      await dashboardPage.clickAddProduct()
      await formPage.waitForForm()

      // The file input has accept="image/*" which prevents non-images
      // Try to set a non-image file and verify it's rejected
      const invalidFilePath = path.join(__dirname, '../../fixtures/files')

      // Note: Browser prevents non-image files due to accept="image/*"
      // This test verifies the client-side validation works
      const fileInput = page.locator('input[accept="image/*"]')
      expect(fileInput).toBeTruthy()

      // The input element should have the correct accept attribute
      const acceptAttr = await fileInput.getAttribute('accept')
      expect(acceptAttr).toBe('image/*')
    })

    // Step 2: Verify we can still complete form with valid image
    await test.step('Complete form with valid image', async () => {
      const imagePath = getTestImagePath()
      await formPage.uploadImage(imagePath)
      const hasPreview = await formPage.hasImagePreview()
      expect(hasPreview).toBe(true)
